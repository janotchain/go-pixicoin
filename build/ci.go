...
var (
	// Files that end up in the pxc*.zip archive.
	pxcArchiveFiles = []string{
		"COPYING",
		executablePath("pxc"),
	}

	// Files that end up in the pxc-alltools*.zip archive.
	...
	debExecutables = []debExecutable{
		...
		{
			Name:        "pxc",
			Description: "pixicoin CLI client.",
		},
		{
			Name:        "puppeth",
...
func doArchive(cmdline []string) {
	var (
		...
		upload = flag.String("upload", "", `Destination to upload the archives (usually "pxcstore/builds")`)
		ext    string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	if err := build.WriteArchive(pxc, pxcArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{pxc, alltools} {
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Where to upload the source package (usually "ppa:pixicoin/pxc")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "pxc-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
type debMetadata struct {
	...
	// go-pixicoin version being built. Note that this
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	// first section contains the pxc binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"pxc":      pxcTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.pxc.nsi", filepath.Join(*workdir, "pxc.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("pxc-" + archiveBasename(*arch, env) + ".exe")
...
func doAndroidArchive(cmdline []string) {
	var (
		...
		upload = flag.String("upload", "", `Destination to upload the archive (usually "pxcstore/builds")`)
	)
	flag.CommandLine.Parse(cmdline)
	env := build.Env()
	...
	if *local {
		...
		os.Rename("pxc.aar", filepath.Join(GOBIN, "pxc.aar"))
		return
	}
	...
	archive := "pxc-" + archiveBasename("android", env) + ".aar"
	os.Rename("pxc.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	archive := "pxc-" + archiveBasename("ios", env)
	...
	if *deploy != "" {
		...
		build.Render("build/pod.podspec", "pxc.podspec", 0755, meta)
		build.MustRunCommand("pod", *deploy, "push", "pxc.podspec", "--allow-warnings", "--verbose")
	}
}
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "pxcstore/builds")`)
