...
// nodeDockerfile is the Dockerfile required to run an pixicoin node.
var nodeDockerfile = `
...
{{end}}
RUN \
  echo 'pxc --cache 512 init /genesis.json' > pxc.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.pixicoin/keystore/ && cp /signer.json /root/.pixicoin/keystore/' >> pxc.sh && \{{end}}
	echo $'pxc --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .BootV4}}--bootnodesv4 {{.BootV4}}{{end}} {{if .BootV5}}--bootnodesv5 {{.BootV5}}{{end}} {{if .Etherbase}}--etherbase {{.Etherbase}} --mine --minerthreads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> pxc.sh

ENTRYPOINT ["/bin/sh", "pxc.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// a pixicoin node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    volumes:
      - {{.Datadir}}:/root/.pixicoin{{if .Ethashdir}}
...
// deployNode deploys a new pixicoin node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 pxc --exec admin.nodeInfo.id attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.pixicoin"],
