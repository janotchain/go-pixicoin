...
#
# The cluster contains a bootnode, a pxc node and multiple swarm nodes, with
...
# static pxc configuration
pxc_IP="192.168.33.3"
pxc_RPC_PORT="8545"
pxc_RPC_URL="http://${pxc_IP}:${pxc_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the pxc and
# swarm nodes
start_bootnode() {
...
# start_pxc_node starts a gcm node with --datadir pointing at <base-dir>/pxc
# and a single, unlocked account with password "pxc"
start_pxc_node() {
  local dir="${base_dir}/pxc"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get pxc account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${pxc_IP}"
    --rpcport   "${pxc_RPC_PORT}"
    --verbosity "6"
  )

  start_node "pxc" "${pxc_IP}" "$(which pxc)" ${args[@]}
}
...
create_account() {
  ...
  pxc --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
