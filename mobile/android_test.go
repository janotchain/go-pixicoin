...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", pxc.LightScryptN, pxc.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.pixicoin", new NodeConfig());
			node.start();
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "pxc-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/pixicoin/pxctest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for pxc and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.pixicoin", "github.com/ethereum/go-ethereum/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "pxc.aar"), "pxc.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "pixicoin", "pxctest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
